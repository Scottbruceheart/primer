...
* put, store, stash, insert, add: Execute a procedure to insert some element into a structure.  Ex: `put(key)->value` in a linked list takes constant, O(1), time and makes the given object the head of the list.[^2]
...
[^2]: Obviously we could keep a tail pointer on a linked list and make the inserts on the tail and not change any time bounds.
[^3]: Usually we use a strict [weak comparator](https://en.wikipedia.org/wiki/Weak_ordering) in this sort of situation, which provides a total ordering on sets.
[^4]: Source: The Vibes, Scott Bruce, when I first read about splay trees around 2008.
[^5]: Knuth's 'The Art Of Computer Programming' has extensive analysis of hash functions, universal hash functions, good entropy mixing, and more.
[^6]: C++ attempts to circumvent this with design: `std::unordered_map` will allow you to insert any number of times without resizing, only resizing when you eventually execute a lookup.  This is probably a good tradeoff in practice, but requires extreme care! A common multithreaded pattern is 'initialize a structure, pass a time barrier, then only issue reads against that structure'. Without knowing that `std::unordered_map` reads aren't _really_ const, the pattern will fail at at least the first read (which causes a long running resize).
[^7]: I actually mean 'only strategies' here. Other variants of collision resolution all come down to one of these two if you tilt your head and squint.  "put it in an alternate structure" or "do another computation to find another slot in this array somewhere".  See cuckoo hashing, robin hood hashing, others.
